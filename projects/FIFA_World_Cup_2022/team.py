...
class Team():
    ...
    def get_players(self,zones, custom_lineup=None) -> List[Player]:
        with open(os.getcwd()+'/projects/FIFA_World_Cup_2022/data.json', 'r', encoding="utf-8") as file:
            data = json.load(file)
        if custom_lineup:
            lineups = custom_lineup
        else:
            with open(os.getcwd()+"/projects/FIFA_World_Cup_2022/lineups.json", "r") as file:
